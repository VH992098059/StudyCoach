...
const ConfigPanel: React.FC<ConfigPanelProps> = ({ form, mode, enabled, paused, onSave, onRunNow, onEnableToggle, onPauseResume }) => {
  return (
    <Card title="定时任务配置" className="section-card">
      <Form form={form} layout="vertical" initialValues={{ mode: 'daily', updateType: 'incremental', minuteOfHour: 0, secondOfMinute: 0 }}>
        ...
        {mode === 'hourly' && (
          <>
            <Form.Item label="分钟" name="minuteOfHour" rules={[{ required: true }]}> 
              <Select style={{ width: 160 }} options={Array.from({ length: 60 }, (_, i) => ({ label: `${i} 分`, value: i }))} />
            </Form.Item>
            <Form.Item label="秒" name="secondOfMinute" rules={[{ required: true }]}> 
              <Select style={{ width: 160 }} options={Array.from({ length: 60 }, (_, i) => ({ label: `${i} 秒`, value: i }))} />
            </Form.Item>
          </>
        )}
        ...
        {mode === 'weekly' && (
          <Form.Item label="周几" name="weekday" rules={[{ required: true }]}> 
            <Select style={{ width: 160 }} options={WEEK_OPTIONS} />
          </Form.Item>
        )}

        {mode === 'monthly' && (
          <Form.Item label="日期" name="dayOfMonth" rules={[{ required: true, type: 'number', min: 1, max: 31 }]}> 
            <Select style={{ width: 160 }} options={Array.from({ length: 31 }, (_, i) => ({ label: `${i + 1} 日`, value: i + 1 }))} />
          </Form.Item>
        )}
        ...
        <Form.Item label="知识库" name="kbId" rules={[{ required: true }]}> 
          <Select placeholder="请选择知识库" options={KB_OPTIONS} style={{ width: 260 }} />
        </Form.Item>

        <Form.Item label="更新内容类型" name="updateType" rules={[{ required: true }]}> 
          <Radio.Group>
        ...
        <Space size={12} className="actions">

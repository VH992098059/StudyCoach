...
type CronLog struct {
	...
	CronID        int64         `gorm:"column:cron_id;index:index_id_time,priority:1"`                                 // 定时任务ID外键，普通联合索引
	Content       string        `gorm:"type:text;column:content"`                                                      // 日志内容
	Level         string        `gorm:"type:varchar(10);column:level"`                                                 // 日志级别 (INFO, ERROR)
	CreateTime    time.Time     `gorm:"column:create_time;index:index_id_time,priority:2"`                             // 创建时间，普通联合索引

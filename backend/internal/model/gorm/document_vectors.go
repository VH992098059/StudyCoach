...
type DocumentVectors struct {
	ID         string    `gorm:"type:uuid;primaryKey;default:gen_random_uuid()"` // 主键，通常为 chunk_id 或文档片段唯一标识（UUID）
	Content    string    `gorm:"column:content;type:text"`                       // 原始文本内容
	Vector     string    `gorm:"column:vector;type:text"`                        // 向量化后的 embedding，JSON 或序列化格式
